...
class EventosView(generics.CreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if evento_data["hora_final"]:
            evento_data["hora_final"] = str(evento_data["hora_final"])[:5]  # "17:00"
        return Response(evento_data, status=200)
    
#Registrar nuevo evento
    @transaction.atomic
    def post(self, request, *args, **kwargs):
        hora_inicio_str = request.data['hora_inicio']
        hora_final_str = request.data['hora_final']

        # Convertimos las horas
        hora_inicio = convertir_hora_12_a_24(hora_inicio_str)
        hora_final = convertir_hora_12_a_24(hora_final_str)
        ...
        try:
            ...
            hora_final_time = datetime.strptime(hora_final, formato).time()
        except ValueError:
...
class EventosViewEdit(generics.CreateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        hora_final_str = request.data.get("hora_final")

        try:
            formato = "%H:%M:%S"
            hora_inicio = datetime.strptime(hora_inicio_str, formato).time()
            hora_final = datetime.strptime(hora_final_str, formato).time()
        except ValueError:
